...
router.get('/export', requireApiKey, async (req, res) => {
  try {
    ...
    // Use originalJson directly to bypass formatResponse middleware
    res.originalJson ? res.originalJson(exportData) : res.json(exportData);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
});

// IMPORT - Import records for a user
router.post('/import', requireApiKey, async (req, res) => {
  try {
    ...
    for (const record of req.body) {
      try {
        // Create new record with user's api_key
        const newRecord = new Record({
          ...record,
